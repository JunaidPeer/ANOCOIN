...
gsigs = 'https://github.com/anocoin-project/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if not os.path.isdir('anocoin'):
        subprocess.check_call(['git', 'clone', args.url, 'anocoin'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/anocoin/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'anocoin='+args.commit, '--url', 'anocoin='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/anocoin-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/anocoin/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/anocoin/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/anocoin-project/anocoin', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/anocoin', exist_ok=True)
    os.chdir('builder/inputs/anocoin')
    if args.pull:
